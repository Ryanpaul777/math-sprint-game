...
// Check local storage for Best Scores, and then set bestScoreArray
...
function scoresToDOM() {
  ...
  updateBestScore();
  // Scroll to top, then go to score page
  ...
  if (playerGuessArray.length == questionAmount) {
    ...
    equationsArray.forEach((equation, index) => {
      if (equation.evaluated === playerGuessArray[index]) {
        // Correct guess; therefore, no penalty
      } else {
        // Incorrect guess, therefore, add penalty
        penaltyTime += 0.5;
      }
    });
...
function selectQuestionAmount(e) {
  ...
  if (questionAmount) {
    ...
    alert('Please select the number of questions you want to answer.');
  }
}
